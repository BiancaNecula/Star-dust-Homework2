...
def run_test(task, test, use_valgrind=False, ):
    ...
    try:
        child = subprocess.run(cmd, shell=False if not task.use_valgrind else True,
                               ...
                               stderr=subprocess.PIPE,
                               timeout=task.timeout if not task.use_valgrind else 100,
                               preexec_fn=limit_process_memory if task.memory and not task.use_valgrind else None,
                               )
    except subprocess.TimeoutExpired as e:
        log_replace('running test     - TLE: {}'.format(str(e)))
        unindent_log()
        return -1
    except Exception as e:
        log_replace('running test     - UPS: {}'.format(str(e)))
        unindent_log()
        return -1
    ...
    if child.returncode != 0:
        if not use_valgrind:
            log_replace(
                'running test     - UPS: exit code = {}'.format(child.returncode))
        elif child.returncode == expected_error_code:
            log_replace('running valgrind - MEM_UPS')
        else:
            bug()
    ...
    if not use_valgrind:
        log_replace('running test     - OK')
    else:
        log_replace('running valgrind - OK')
    unindent_log()
    return 0
...
def check_test(grader, test):
    ...
    try:
        ...
        log_replace('running check    - UPS: {}'.format(str(e)))
        unindent_log()
        return -1

    if child.returncode != 0:
        log_replace(
            'running check    - WA: {}'.format(extract_stderr(child)))
        unindent_log()
        return child.returncode
    ...
    log_replace('running check    - OK{}'.format((': {}'.format(warnings) if len(warnings) > 0 else '')))
    unindent_log()
    return 0
...
def run_task(task):
    ...
    for test in task.tests:
        ...
        stages = [
            ...
            lambda: task.use_valgrind and run_test(
                task, test, use_valgrind=True)
        ]
        code = 0
        for stage in stages:
            code = stage()
